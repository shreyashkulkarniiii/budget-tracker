      ...
      <View style={styles.expenseMain}>
        <View
          style={[
            ...
            { backgroundColor: Colors.dark.categories[item.category as keyof typeof Colors.dark.categories] },
          ]}
        />
        ...
        <View style={styles.activeBadgeRow}>
          <View style={styles.activeBadge}>
            <Text style={styles.activeBadgeText}>
              {filterLabels.find((f) => f.key === dateFilter)?.label}
              {dateFilter === 'custom' && customFrom && customTo ? `: ${customFrom} → ${customTo}` : ''}
            </Text>
            <TouchableOpacity onPress={() => { setDateFilter('all'); setCustomFrom(''); setCustomTo(''); }}>
              <X size={14} color={Colors.dark.primary} />
            </TouchableOpacity>
          </View>
        </View>
      )}
          ...
          <View style={styles.modalSheet} onStartShouldSetResponder={() => true}>
            ...
            {filterLabels.map((f) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  setDateFilter(f.key);
                  if (f.key !== 'custom') setShowFilters(false);
                }}
              >
                <Text style={[styles.filterOptionText, dateFilter === f.key && styles.filterOptionTextSelected]}>
                  {f.label}
                </Text>
...
const styles = StyleSheet.create({
  ...
  searchBar: {
    ...
    paddingHorizontal: Spacing.md, borderWidth: 1, borderColor: Colors.dark.border, gap: Spacing.sm,
  },
  ...
  activeBadge: {
    ...
    borderWidth: 1, borderColor: Colors.dark.primary, alignSelf: 'flex-start',
  },
  ...
  modalSheet: {
    backgroundColor: Colors.dark.surface, borderTopLeftRadius: BorderRadius.xl,
  ...
  customLabel: { fontSize: Typography.sizes.sm, color: Colors.dark.textSecondary, marginBottom: Spacing.xs, marginTop: Spacing.sm },
  customInput: {
  ...
  expenseCard: {
    ...
    padding: Spacing.md, marginBottom: Spacing.md, borderWidth: 1, borderColor: Colors.dark.border,
  },
